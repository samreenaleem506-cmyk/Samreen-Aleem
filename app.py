...
if select=="Home":
    ...
    df2 = pdata[["matches" , "innings" , "high_score" , "average" , "100" , "50" , "4s" , "6s"]]
    st.dataframe(df2)
    ...
    col1.metric("Total Players" , len(cdata["player"].unique()))
...
#_________________Player comparison____________________#
    ...
    players = st.multiselect("compare countries" , df["country"] , default = df["country"].head(16))

    compare = df [df["country"] . isin(players)]

    fig = px.scatter(compare , x="strike_rate" , y="average" , size = "runs" , color = "country" , hover_name = "country")
...
st.markdown(f"[LinkedIn]({url1})")
url2 = "https://github.com/samreenaleem506-cmyk/"
st.markdown(f"[GitHub]({url2})")
col1,col2,col3,col4 = st.columns(4)
